...
// ParseSelectValueWithCache 解析字段值
func (t *fieldNodeTree) ParseSelectValueWithCache(key, selectScene string, el interface{}) {
	...
	TakeValueOfPointerValue: //这里主要是考虑到有可能用的不是一级指针，如果是***int 等多级指针就需要不断的取值
		...
		for i := 0; i < typeOf.NumField(); i++ {
			...
			if !find {
				...
				tag, find = tagCache.getTag(jsonTag, selectScene, pkgInfo, true, typeOf.Field(i).Name, nil)
			}
		...
		TakeFieldValue:
			...
			tree.ParseSelectValueWithCache(tag.UseFieldName, selectScene, value.Interface())
	...
	takeVMap:
		...
		takeValMap:
			...
			if mapIsNil {
				...
				nodeTree.ParseSelectValueWithCache(k, selectScene, val.Interface())
				t.AddChild(nodeTree)
			}
		}
	...
	case reflect.Slice, reflect.Array:
		...
		takeValSlice:
			...
			if sliceIsNil {
				...
				node.ParseSelectValueWithCache("", selectScene, valueOf.Index(i).Interface())
				t.AddChild(node)
			}
		}
	}
