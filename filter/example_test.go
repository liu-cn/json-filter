...
func ExampleEqualJSON() {
	...
	fmt.Println(EqualJSON(json1, json2))        //结构相等且值相等，返回true
	fmt.Println(EqualJSON(json1, json2, json3)) //判断多个，需要全部等价才返回true
	fmt.Println(EqualJSON(json3, json4))        //结构相等但是值不相等，不等价
	fmt.Println(EqualJSON(json3, json5))        //结构不相等不等价
