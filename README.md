...
​		1.1 [select() 选择器，选择过滤](#select() 选择器，选择过滤)

   	 1.2 [omit() 选择器， 排除过滤](omit() 选择器， 排除过滤)

2. [$any标识符 任意场景解析](#$any标识符 任意场景解析)

3. [过滤后的Filter结构体的方法](#过滤后的Filter结构体的方法)

​	    3.1  [Filter.Interface()](#Filter.Interface())  

   	 3.2  [Filter.MustJSON()](#Filter.MustJSON())

4. [高级用法](#高级用法)

​		 4.1 [对于map的过滤](#对于map的过滤)

​		4.2 [对于切片/数组的直接过滤](#对于切片/数组的直接过滤)

​		4.3  [匿名结构体的过滤](#匿名结构体的过滤)

​			4.3.1  [展开结构体](#展开结构体)

​			4.3.2  [不想展开结构体](#不想展开结构体)

​	4.4 [极其复杂的场景，深层嵌套 结构体嵌套结构体，嵌套切片，嵌套map，嵌套指针，等等](#极其复杂的场景，深层嵌套 结构体嵌套结构体，嵌套切片，嵌套map，嵌套指针，等等)
